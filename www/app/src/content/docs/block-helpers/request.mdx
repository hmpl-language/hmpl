...
:::note
The `"error"` trigger value overlaps with `"rejected"` and any HTTP status codes in the range `400` to `599`. This allows authors to define a fallback indicator without the need to enumerate all possible error conditions individually.
:::
